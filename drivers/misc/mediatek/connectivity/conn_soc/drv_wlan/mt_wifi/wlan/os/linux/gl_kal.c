...
/* filp = filp_open("/system/etc/firmware/"CFG_FW_FILENAME"_MT6582", O_RDONLY, 0); */
	...
	kalMemCopy(aucFilePath, "/system/etc/firmware/" CFG_FW_FILENAME "_AD", sizeof("/system/etc/firmware/" CFG_FW_FILENAME "_AD"));
	filp = filp_open(aucFilePath, O_RDONLY, 0);
	if (!IS_ERR(filp))
		goto open_success;
#endif
	kalMemZero(aucFilePath, sizeof(aucFilePath));
	kalMemCopy(aucFilePath, "/system/etc/firmware/" CFG_FW_FILENAME "_", strlen("/system/etc/firmware/" CFG_FW_FILENAME "_"));
