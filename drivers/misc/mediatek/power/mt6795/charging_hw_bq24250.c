...
//		if(charging_led_flag==1)
		...
		battery_log(BAT_LOG_CRTI, "%s disable charging led, flag=%d\r\n", __func__, charging_led_flag);
	}else
	{
//		if(charging_led_flag==0)
		...
		battery_log(BAT_LOG_CRTI, "%s enable charging led flag=%d\r\n", __func__, charging_led_flag);
	}
	 ...
	 */
	   ...
	   if(ret == 0)
	   {
		   *(kal_bool*)(data) = 0; // battery exist
		   battery_log(BAT_LOG_CRTI,"[charging_get_battery_status] battery exist.\n");
	   }
	   else
	   {
