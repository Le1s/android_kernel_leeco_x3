...
int PMIC_IMM_GetOneChannelValue(upmu_adc_chl_list_enum dwChannel, int deCount, int trimd)
{
	...
	*/
	...
	do
	{
		...
		udelay(1);
		    ...
		    case AUX_TSENSE_32_MD:
		            ...
		            {
				battery_log(BAT_LOG_CRTI,  "[IMM_GetOneChannelValue_PMIC] (%d) Time out!\n", dwChannel);
				break;
		            }
	                }
		...
		if (Enable_BATDRV_LOG == 2) {
			battery_log(BAT_LOG_CRTI,  "[AUXADC] u4channel[%d]=%d.\n",
				dwChannel, ret_data);
		}
	...
	case AUX_VISMPS_1_AP:
	    ...
	    break;
	case AUX_ADCVIN0_AP:
		battery_log(BAT_LOG_CRTI,  "[AUXADC]AUX_ADCVIN0_AP\n");
	...
	if (Enable_BATDRV_LOG == 2) {
		battery_log(BAT_LOG_CRTI,  "[AUXADC] adc_result_temp=%d, adc_result=%d, r_val_temp=%d.\n",
