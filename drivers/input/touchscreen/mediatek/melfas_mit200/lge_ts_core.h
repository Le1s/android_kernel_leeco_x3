...
do {  if (unlikely(touch_debug_mask_ & DEBUG_TRACE)) \
TOUCH_DEBUG_MSG("\n");
} while (0)

#define TOUCH_FW_MSG(fmt, args...) \
do {  if (unlikely(touch_debug_mask_ & DEBUG_FW_UPGRADE)) TOUCH_DEBUG_MSG(fmt, ##args); } while (0)

#define TOUCH_POWER_MSG(fmt, args...) \
do {  if (unlikely(touch_debug_mask_ & DEBUG_POWER)) TOUCH_DEBUG_MSG(fmt, ##args); } while (0)
...
#define EXTERNAL_FW_PATH    "/system/etc/firmware/"
